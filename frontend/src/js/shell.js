...
export function bindShell() {
  ...
  $("#content-body").addEventListener("click", async (e) => {
    ...
    if (control && handleSettingControl(control, { commit })) return;

    const actEl = e.target.closest("[data-act]");
    if (actEl && actEl.closest(".settings")) {
      const blocked = ["add-folder", "remove-folder", "scan-now", "rescan-folder"];
      await handleSettingsAction(actEl, { commit, rescan: () => doRescan({ manual: true }) });
      if (blocked.includes(actEl.dataset.act)) return;
      if (actEl.dataset.act?.startsWith("rule") || actEl.dataset.act?.startsWith("preset")) {
        renderContent();
        renderHeader();
      }
      return;
