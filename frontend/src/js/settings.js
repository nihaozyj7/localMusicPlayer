...
import { applyResolvedTheme, listThemes, resolvedGlassBlur } from "./theme.js";
...
export async function handleSettingsAction(actEl, ctx = {}) {
  ...
  switch (act) {
    ...
    case "add-folder": {
      let path = null;
      if (isWails) path = await backend.pickDirectory();
      if (!path) {
      ...
      ctx.commit?.();
      toast(`已添加文件夹：${path}`, { tone: "success" });
      ctx.rescan?.();
      break;
    }
    case "remove-folder": {
    ...
    case "open-theme-dir":
      if (isWails) backend.revealInExplorer("themes");
      toast("主题目录：frontend/src/styles/themes/", { duration: 2600 });
      break;
    case "theme-help":
      openModal({
        title: "添加自定义主题",
        ...
        body: `
          <div class="setting__hint setting__hint--steps">
            1. 复制 <b>themes/_template.css</b> 为目标文件，例如 <b>themes/sunset.css</b><br />
            2. 把选择器改成 <b>:root[data-theme="sunset"]</b><br />
            3. 在 <b>styles/index.css</b> 末尾加一行 <b>@import "./themes/sunset.css";</b><br />
            4. 回到这里即可看到新主题卡片
          </div>
          <div class="setting__hint">核心令牌：--glass-bg / --glass-blur / --accent / --text-1 / --bg-app</div>`,
        okText: "知道了",
        cancelText: "关闭",
        onOk: () => true,
      });
      break;
    case "clear-cache":
      toast("缓存已清空（预览模式不会真的写入磁盘）", { tone: "success" });
      break;
    default:
      break;
  }
