import{s as c,t as A,c as N,i as l,a as T,b as B,d as z,n as P}from"./index-DIncFb_9.js";import{backend as p,isWails as $}from"./bridge-B8JBWU-B.js";let L=!1,i=null,v=null,u=null,o=[],n=new Set,g=null,S=!1;function j(){if(!L){if(L=!0,i=document.getElementById("cover-layer"),!i)throw new Error("缺少 #cover-layer 容器");v=i.querySelector("#cover-layer-body"),i.addEventListener("click",e=>{if(e.target.closest("[data-cover-close]")||e.target===i){H();return}const t=e.target.closest("[data-set-act]");if(t){const r=Number(t.closest("[data-set-index]")?.dataset.setIndex);t.dataset.setAct==="use"&&J(r),t.dataset.setAct==="remove"&&U(r);return}const a=e.target.closest("[data-cover-act]")?.dataset.coverAct;a&&(a==="search"&&R(),a==="local"&&W(),a==="toggle"&&O(Number(e.target.closest("[data-cover-idx]")?.dataset.coverIdx)),a==="select-all"&&Q(),a==="apply"&&G(),a==="carousel"&&V(),a==="embed"&&X(),a==="open-cache"&&p.coverOpenCacheDir("covers"))}),i.addEventListener("keydown",e=>{if(e.key!=="Escape")return;const t=i.querySelector("#cover-keyword");if(t&&document.activeElement===t&&t.value.trim()){t.value="";return}H()})}}function ee(e){j(),u=e,o=[],n=new Set,g=c.coverSets.get(e)||null;const t=c.songs.find(a=>a.id===e);if(!t){A("这首歌不在本地曲库里，无法更换封面",{tone:"warning"});return}D(t),i.hidden=!1,requestAnimationFrame(()=>i.setAttribute("data-state","opened")),F(),I()}async function I(){if(!(c.coverProviders?.length||!$()))try{const e=await p.coverProviders();Array.isArray(e?.providers)&&e.providers.length&&(c.coverProviders=e.providers)}catch{}}function E(){const e=c.coverProviders||[];return e.length?e.join(" / "):"iTunes / 网易云 / QQ 音乐 / Deezer / MusicBrainz"}function H(){i&&(i.setAttribute("data-state","closed"),setTimeout(()=>{i.getAttribute("data-state")==="closed"&&(i.hidden=!0)},B()+40))}async function F(){if(!(!$()||!u))try{const e=await p.coverList(u);e&&Array.isArray(e.items)&&(g=e,z(u,e),q(),w(),P())}catch(e){s(`读取现有封面失败：${e?.message??e}`)}}function D(e){const t=N(e);v.innerHTML=`
    ...
    </div>`,q(),w(),_()}function s(e){const t=v?.querySelector("#cover-status");t&&(t.textContent=e||"")}function q(){const e=v?.querySelector("#cover-set");if(!e)return;const t=g?.items||[],a=g?.embedded||[],r=Number(g?.active)||0;if(!t.length&&!a.length){e.innerHTML='<div class="cover-set__empty">还没有换过封面，这首歌现在用的是文件自带的封面。</div>';return}const b=t.map((d,m)=>`
      <div class="cover-set__item" data-set-index="${m}" data-active="${m===r}">
        <img src="${h(d.preview)}" alt="" />
        ${m===r?'<span class="cover-set__badge">当前</span>':""}
        <div class="cover-set__ops">
          <button class="btn btn--xs" type="button" data-set-act="use" ${m===r?"disabled":""}>
    ...
    </button>`}}function O(e){const t=o[e];if(!t)return;n.has(t)?n.delete(t):n.add(t);const a=v?.querySelector(`[data-cover-idx="${e}"]`);if(a){const r=n.has(t);a.dataset.selected=String(r),a.setAttribute("aria-checked",String(r))}C()}function Q(){n.size===o.length?n.clear():o.forEach(e=>n.add(e)),_()}async function R(){if(S)return;if(!$()){s("浏览器预览下没有联网封面后端，请在应用里试");return}S=!0;const e=o.filter(a=>a.local);o=[...e],n=new Set(e),_();const t=(v?.querySelector("#cover-keyword")?.value||"").trim();s(t?`正在按「${t}」同时查询多个来源（${E()}）…`:`正在同时查询多个来源（${E()}）…`);try{const a=t?{keyword:t}:{},r=await p.coverLookupSongAll(u,a),b=Array.isArray(r)?r.filter(f=>f?.ok&&f.preview):[];if(b.length){o=[...e,...b.map(d=>({...d,local:!1}))];const f=[...new Set(b.map(d=>d.provider).filter(Boolean))];s(`找到 ${b.length} 张（来源：${f.join(" / ")||"未知"}），勾选后点「应用」`)}else{o=[...e];const f=Array.isArray(r)?r.find(d=>d?.message)?.message:"";s(f||"没有找到匹配的封面（也可能是匹配到的都是空白图，已自动丢弃）")}}catch(a){s(`搜索失败：${a?.message??a}`)}finally{S=!1,_()}}async function W(){if(!$()){s("浏览器预览下没有系统文件选择器，请在应用里试");return}s("正在读取图片…");try{const e=await p.coverPickLocal();if(!e||e.cancelled){s("");return}if(!e.ok||!e.preview){s(e?.message||"这张图片没法用作封面");return}const t={preview:e.preview,provider:e.provider||"本地图片",source:e.source||"",width:e.width,height:e.height,local:!0};o.unshift(t),n.add(t),_(),s(`已加入本地图片${e.source?`（${e.source}）`:""}，确认后点「应用」`)}catch(e){s(`选择图片失败：${e?.message??e}`)}}async function G(){const e=o.filter(t=>n.has(t)).map(t=>t.preview).filter(Boolean);if(!e.length){s("先勾选至少一张封面");return}await k(()=>p.coverAddMany(u,e,c.config.embedMeta===!0))}async function J(e){Number.isFinite(e)&&await k(()=>p.coverSetActive(u,e))}async function K(e){e?.preview&&await k(()=>p.coverAdd(u,"",e.preview,c.config.embedMeta===!0))}async function U(e){Number.isFinite(e)&&await k(()=>p.coverRemove(u,e))}async function k(e){if(!$()){s("浏览器预览下没有封面后端，请在应用里试");return}s("正在保存…");try{const t=await e();t&&Array.isArray(t.items)&&(g=t,z(u,t),q(),w(),P()),s(t?.message||"已更新封面"),A(t?.message||"封面已更新",{tone:"success",duration:1800})}catch(t){s(`保存失败：${t?.message??t}`)}}function V(){c.config.coverCarousel=!c.config.coverCarousel,w(),T(),A(c.config.coverCarousel?`已开启封面轮播（每 ${Number(c.config.coverCarouselInterval)||10} 秒换一张）`:"已关闭封面轮播",{duration:1600})}function X(){c.config.embedMeta=!c.config.embedMeta,w(),T(),A(c.config.embedMeta?"之后的封面会写进歌曲文件本身":"封面只保存在缓存目录（不改动音乐文件）",{duration:2200})}function y(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function h(e){return y(e)}export{H as closeCoverPanel,ee as openCoverPanel};
