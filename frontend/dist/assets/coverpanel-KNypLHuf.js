import{s as c,t as A,i as l,c as T,a as N,b as z,n as P}from"./index-D2d2iLas.js";import{backend as p,isWails as $}from"./bridge-B8JBWU-B.js";import{q as B}from"./index-un_4l_TW.js";let L=!1,i=null,v=null,u=null,o=[],n=new Set,g=null,S=!1;function j(){if(!L){if(L=!0,i=document.getElementById("cover-layer"),!i)throw new Error("缺少 #cover-layer 容器");v=i.querySelector("#cover-layer-body"),i.addEventListener("click",e=>{if(e.target.closest("[data-cover-close]")||e.target===i){H();return}const t=e.target.closest("[data-set-act]");if(t){const a=Number(t.closest("[data-set-index]")?.dataset.setIndex);t.dataset.setAct==="use"&&J(a),t.dataset.setAct==="remove"&&U(a);return}const r=e.target.closest("[data-cover-act]")?.dataset.coverAct;r&&(r==="search"&&R(),r==="local"&&W(),r==="toggle"&&O(Number(e.target.closest("[data-cover-idx]")?.dataset.coverIdx)),r==="select-all"&&Q(),r==="apply"&&G(),r==="carousel"&&V(),r==="embed"&&X(),r==="open-cache"&&p.coverOpenCacheDir("covers"))}),i.addEventListener("keydown",e=>{if(e.key!=="Escape")return;const t=i.querySelector("#cover-keyword");if(t&&document.activeElement===t&&t.value.trim()){t.value="";return}H()})}}function te(e){j(),u=e,o=[],n=new Set,g=c.coverSets.get(e)||null;const t=c.songs.find(r=>r.id===e);if(!t){A("这首歌不在本地曲库里，无法更换封面",{tone:"warning"});return}D(t),i.hidden=!1,requestAnimationFrame(()=>i.setAttribute("data-state","opened")),F(),I()}async function I(){if(!(c.coverProviders?.length||!$()))try{const e=await p.coverProviders();Array.isArray(e?.providers)&&e.providers.length&&(c.coverProviders=e.providers)}catch{}}function E(){const e=c.coverProviders||[];return e.length?e.join(" / "):"iTunes / 网易云 / QQ 音乐 / Deezer / MusicBrainz"}function H(){i&&(i.setAttribute("data-state","closed"),setTimeout(()=>{i.getAttribute("data-state")==="closed"&&(i.hidden=!0)},N()+40))}async function F(){if(!(!$()||!u))try{const e=await p.coverList(u);e&&Array.isArray(e.items)&&(g=e,z(u,e),C(),w(),P())}catch(e){s(`读取现有封面失败：${e?.message??e}`)}}function D(e){const t=B(e);v.innerHTML=`
    <div class="cover-panel">
    ...
    </div>`,C(),w(),_()}function s(e){const t=v?.querySelector("#cover-status");t&&(t.textContent=e||"")}function C(){const e=v?.querySelector("#cover-set");if(!e)return;const t=g?.items||[],r=g?.embedded||[],a=Number(g?.active)||0;if(!t.length&&!r.length){e.innerHTML='<div class="cover-set__empty">还没有换过封面，这首歌现在用的是文件自带的封面。</div>';return}const b=t.map((d,m)=>`
      ...
      </button>`}).join(""),q()}}function q(){const e=v?.querySelector("#cover-selectbar");if(e){if(!o.length){e.hidden=!0,e.innerHTML="";return}e.hidden=!1,e.innerHTML=`
