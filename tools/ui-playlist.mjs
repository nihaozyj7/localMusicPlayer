...
async function boxOf(selector, nth) {
  return evalJs(
    "const list = document.querySelectorAll(" + JSON.stringify(selector) + "); const el = list[" + nth + "]; if (!el) return null; const r = el.getBoundingClientRect(); return { x: r.left + r.width / 2, y: r.top + r.height / 2 };"
  );
}
...
try {
  ...
  await go(`const store = await import("/js/store.js"); store.state.view = "queue"; store.state.playlistId = null; store.state.query = ""; store.state.playMode = "shuffle"; store.state.queue = store.state.songs.slice(0, 6).map((s) => s.id); store.state.currentIndex = 0; store.state.currentId = store.state.queue[0]; store.commit(); await new Promise((r) => setTimeout(r, 360)); return true;`);
  const info = await go(`const store = await import("/js/store.js"); return { queue: store.state.queue.slice(), handles: document.querySelectorAll(".track__handle").length, staticHead: document.querySelectorAll(".tracks__sort[data-static]").length };`);
  check(
  ...
  const after = await go(`const store = await import("/js/store.js"); return { queue: store.state.queue.slice(), playMode: store.state.playMode, dom: Array.from(document.querySelectorAll(".tracks__body .track")).map((n) => n.dataset.id) };`);
...
/* ==========================================================================
   ...
   ========================================================================== */
try {
  await go(`const store = await import("/js/store.js"); store.state.view = "playlist"; store.state.playlistId = "pl_late_night"; store.state.query = ""; store.state.playlistSelecting = false; store.state.selectedIds = new Set(); store.commit(); await new Promise((r) => setTimeout(r, 360)); return true;`);
  const tools = await go(`return { select: Boolean(document.querySelector('[data-tool="pl-select"]')), add: Boolean(document.querySelector('[data-tool="pl-add"]')), rows: document.querySelectorAll(".track").length };`);
  ...
  await sleep(360);
  const selMode = await go(`return { checks: document.querySelectorAll(".track__check").length, selectBtn: document.querySelector('[data-tool="pl-select"]') ? 1 : 0 };`);
  ...
  await sleep(360);
  const picked = await go(`const store = await import("/js/store.js"); const info = document.querySelector(".toolbar__selinfo"); return { selected: store.state.selectedIds.size, info: info ? info.textContent.trim() : "" };`);
  check("点行即勾选，工具条显示已选数量", picked.selected === 2, JSON.stringify(picked));

  const beforeCount = await go(`const store = await import("/js/store.js"); return store.state.playlists.find((p) => p.id === "pl_late_night").songIds.length;`);
  await go(`document.querySelector('[data-tool="sel-remove"]').click(); return true;`);
  await sleep(420);
  const afterRemove = await go(`const store = await import("/js/store.js"); return { count: store.state.playlists.find((p) => p.id === "pl_late_night").songIds.length, selecting: store.state.playlistSelecting };`);
  check(
    "移除所选：歌单少 2 首并退出多选",
  ...
  await sleep(420);
  const dialog = await go(`const modal = document.querySelector(".modal"); return { open: Boolean(modal), rows: document.querySelectorAll(".addsongs__row").length, ok: Boolean(modal && modal.querySelector('[data-act="ok"]')) };`);
  check("点添加打开弹层并列出所有歌曲", dialog.open && dialog.rows > 0 && dialog.ok, JSON.stringify(dialog));

  await go(`const boxes = Array.from(document.querySelectorAll("[data-song-check]:not(:disabled)")); boxes.slice(0, 2).forEach((b) => (b.checked = true)); document.querySelector(".modal [data-act='ok']").click(); return true;`);
  await sleep(480);
  const afterAdd = await go(`const store = await import("/js/store.js"); return { count: store.state.playlists.find((p) => p.id === "pl_late_night").songIds.length, modalOpen: Boolean(document.querySelector(".modal")) };`);
  check("勾选后加入歌单", afterAdd.count === afterRemove.count + 2 && afterAdd.modalOpen === false, JSON.stringify(afterAdd));
} catch (err) {
...
/* ==========================================================================
   ...
   ========================================================================== */
try {
  await go(`document.querySelector("#btn-settings").click(); await new Promise((r) => setTimeout(r, 460)); return true;`);
  const nav = await go(`const nav = document.querySelector(".settings__nav"); const wrap = document.querySelector(".settings"); const n = nav.getBoundingClientRect(); const w = wrap.getBoundingClientRect(); const cs = getComputedStyle(nav); return { delta: Math.round((n.left + n.right) / 2 - (w.left + w.right) / 2), top: cs.top, position: cs.position, labels: Array.from(document.querySelectorAll(".settings__nav-item")).map((b) => b.textContent.trim()) };`);
  check(
    "设置导航条居中且吸附锚点 top=0",
  ...
  const aiToggle = await go(`const label = Array.from(document.querySelectorAll(".setting__label")).find((n) => n.textContent.includes("自动匹配歌词时使用")); return { found: Boolean(label), card: label ? label.closest(".card").querySelector(".card__title").textContent.trim() : "", toggle: Boolean(label && label.closest(".setting").querySelector('[data-toggle="aiLyricsClean"]')) };`);
  check("AI 相关里有「自动匹配歌词时使用 AI 清洗元数据」开关", aiToggle.found && aiToggle.toggle && aiToggle.card === "AI 相关", JSON.stringify(aiToggle));
  ...
  await go(`document.querySelector('[data-act="skin-help"]').click(); await new Promise((r) => setTimeout(r, 420)); return true;`);
  const guide = await go(`const modal = document.querySelector(".modal"); return { open: Boolean(modal), boxes: document.querySelectorAll("[data-prompt-text]").length, copy: document.querySelectorAll("[data-copy-prompt]").length };`);
  check("点自定义打开 AI 提示词引导弹层（皮肤 + 主题两份提示词）", guide.open && guide.boxes === 2 && guide.copy === 2, JSON.stringify(guide));
  await go(`const btn = document.querySelector(".modal [data-act='cancel']"); if (btn) btn.click(); await new Promise((r) => setTimeout(r, 300)); const close = document.querySelector("[data-settings-close]"); if (close) close.click(); return true;`);
  await sleep(400);
} catch (err) {
...
try {
  ...
  await sleep(320);
  const narrow = await go(`const side = document.querySelector(".sidebar"); return { width: Math.round(side.getBoundingClientRect().width), texts: document.querySelectorAll(".navitem__text").length, hidden: Array.from(document.querySelectorAll(".navitem__text")).filter((n) => getComputedStyle(n).display === "none").length, badgeHidden: Array.from(document.querySelectorAll(".navitem__badge")).filter((n) => getComputedStyle(n).display === "none").length };`);
  check(
...
/* ==========================================================================
   ...
   ========================================================================== */
try {
  const chrome = await go(`const main = document.querySelector(".transport__btn--main"); const fill = document.querySelector(".playerbar__progress .slider__fill"); const cs = main ? getComputedStyle(main) : null; const fs = fill ? getComputedStyle(fill) : null; return { bg: cs ? cs.backgroundColor : "", filter: fs ? fs.filter : "", width: main ? Math.round(main.getBoundingClientRect().width) : 0, height: main ? Math.round(main.getBoundingClientRect().height) : 0 };`);
  check(
    "底栏播放/暂停按钮无背景色（尺寸保留）",
    /rgba\(0, 0, 0, 0\)|transparent/.test(chrome.bg) && chrome.width >= 30 && chrome.height >= 30,
    JSON.stringify(chrome)
  );
  check("进度条默认压暗（brightness < 1）", /brightness/.test(chrome.filter), JSON.stringify({ filter: chrome.filter }));
} catch (err) {
